...
INPUT_FILES_FOLDER, OUTPUT_FILES_FOLDER = "data/input/", "data/output/"
...
def application_train_test(num_rows=None, nan_as_category=False):
    # Read data and merge
    with open(INPUT_FILES_FOLDER+"application_train.csv", 'r') as f:
        df = pd.read_csv(f, nrows=num_rows)
#     df = pd.read_csv('data/input/application_train.csv', nrows=num_rows)
    with open(INPUT_FILES_FOLDER+"application_test.csv", 'r') as f:
        test_df = pd.read_csv(f, nrows=num_rows)
#     test_df = pd.read_csv('data/input/application_test.csv', nrows=num_rows)
    ...
    # df = df.loc[df['NAME_CONTRACT_TYPE_Cash loans'] == 0,]
    del test_df
    gc.collect()
    return df
...
def bureau_and_balance(num_rows=None, nan_as_category=True):
    with open(INPUT_FILES_FOLDER+"bureau.csv", 'r') as f:
        bureau = pd.read_csv(f, nrows=num_rows)
    # bureau = pd.read_csv('data/input/bureau.csv', nrows=num_rows)
    with open(INPUT_FILES_FOLDER+"bureau_balance.csv", 'r') as f:
        bb = pd.read_csv(f, nrows=num_rows)
...
def previous_applications(num_rows=None, nan_as_category=True):
    with open(INPUT_FILES_FOLDER+"previous_application.csv", 'r') as f:
        prev = pd.read_csv(f, nrows=num_rows)
...
def pos_cash(num_rows=None, nan_as_category=True):
    with open(INPUT_FILES_FOLDER+"POS_CASH_balance.csv", 'r') as f:
        pos = pd.read_csv(f, nrows=num_rows)
...
def installments_payments(num_rows=None, nan_as_category=True):
    with open(INPUT_FILES_FOLDER+"installments_payments.csv", 'r') as f:
        ins = pd.read_csv(f, nrows=num_rows)
...
def credit_card_balance(num_rows=None, nan_as_category=True):
    with open(INPUT_FILES_FOLDER+"credit_card_balance.csv", 'r') as f:
        cc = pd.read_csv(f, nrows=num_rows)
...
def main(debug=False):
    ...
    with timer("Run LightGBM with kfold"):
        ...
        gc.collect()
        print(df.shape)
        df.to_csv(INPUT_FILES_FOLDER+"train_test_full_dataset.csv", index=False)
        # feat_importance = kfold_lightgbm(df, num_folds=5, stratified=False, debug=debug)


if __name__ == "__main__":
    submission_file_name = OUTPUT_FILES_FOLDER + time.strftime("cash_%Y%m%d_%H%M%S", time.gmtime()) + ".csv"
